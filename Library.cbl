******************************************************************
      ...
      ******************************************************************
       ...
       01  BORROW-BOOK-SYS.
           ...
           05 BORROW-DATE-FORMATTED   PIC X(21).
       ...
       01  NoZero-FINE-BAHT   PIC Z,ZZ9.
       01  Nozero-LATE-DAYS   PIC ZZ9.
       ...
       01 WS-BOOK-RETURNED PIC A VALUE "N".
               ...
               DISPLAY "Status     : " BOOK-STATUS
                      IF BOOK-STATUS = "A"
                         DISPLAY "This book is available"
                                 " for borrowing."
                         MOVE BORROW-ISBN TO BORROW-ISBN
                             ELSE
       ...
       RETURN-BOOK.
            ...
            ELSE
               ...
               ELSE
                  ...
                  move LATE-DAYS TO Nozero-LATE-DAYS
                  move FINE-BAHT TO NoZero-FINE-BAHT
                  DISPLAY "Late days : " Nozero-LATE-DAYS
                  DISPLAY "Fine (Baht): " NoZero-FINE-BAHT
           ...
           PERFORM UNTIL WS-END-OF-FILE
               READ BORROW-FILE
                   ...
                   IF BORROW-ID-FILE = CURRENT-BORROW-ID
                          MOVE RETURN-DATE(1:4) TO BRW-YYYY
                          MOVE RETURN-DATE(6:2) TO BRW-MM
                          MOVE RETURN-DATE(9:2) TO BRW-DD
      ...
      *>      แปลงวันที่
           ...
           MOVE RETURN-CURRENT-DATE-TIME(7:2) TO RETURN-DAY
            ...
            COMPUTE INT-RTN = FUNCTION INTEGER-OF-DATE
                 (RETURN-YEAR * 10000 + RETURN-MONTH * 100 + RETURN-DAY)
           COMPUTE LATE-DAYS = INT-RTN - INT-BRW
           IF LATE-DAYS < 0
              MOVE 0 TO LATE-DAYS
           END-IF
      ...
      *> Get current date and format it properly
       GET-CURRENT-DATE.
           ...
           *> จัดรูปแบบวันที่ยืม (YYYY-MM-DD HH:MM:SS)
      ...
      *> Convert borrow date into integer days (ใช้วันที่ปัจจุบัน)
      ...
      *> Add due date depending on category
           IF BOOK-CATEGORY = "Cartoon"
               COMPUTE CURRENT-DATE-INT = CURRENT-DATE-INT + 5
           ELSE
               COMPUTE CURRENT-DATE-INT = CURRENT-DATE-INT + 14
           END-IF

           MOVE FUNCTION DATE-OF-INTEGER(CURRENT-DATE-INT)
           TO RETURN-DATE-PRE
           ...
           STRING RETURN-YEAR-PRE DELIMITED BY SIZE
                  ...
                  INTO RETURN-DATE      *> เก็บวันที่ครบกำหนด
           END-STRING
      ...
      *> RETURN-DATE จะเป็นวันที่ครบกำหนดคืน ไม่ใช่ "NULL"
           ...
           *> Display borrow information
           ...
           DISPLAY "Borrowed Date : " BORROW-DATE-FORMATTED(1:10)
           DISPLAY "Borrowed Time : " BORROW-DATE-FORMATTED(12:8)
           DISPLAY "Return Date   : " RETURN-DATE
      ...
      *> History - Show borrow and return history
           ...
           ELSE
             ...
             DISPLAY "                         BORROW & RETURN HISTORY"
               ...
               *> Reset counter
               ...
               DISPLAY "======================================"
                       "======================================"
               DISPLAY "Borrow ID       ISBN                 "
                       "Date Borrowed         Return Status"
               DISPLAY "======================================"
                       "======================================"
                   ...
                   READ BORROW-FILE
                       ...
                       NOT AT END
                           IF BORROW-MEM-ID = USER-MEM-ID
                               ...
                               *> ตรวจสอบในไฟล์ RETURN ว่าหนังสือเล่มนี้ถูกคืนแล้วหรือยัง
                               PERFORM CHECK-IF-RETURNED

                               IF WS-BOOK-RETURNED = "Y"
                                   DISPLAY BORROW-ID-FILE " "
                                         BORROW-BOOK-ISBN(1:17) " "
       ...
       CHECK-IF-RETURNED.
           ...
           *> เปิดไฟล์ RETURN เพื่อตรวจสอบ
           OPEN INPUT RETURN-FILE.

           *> วนลูปอ่านไฟล์ RETURN ทั้งหมด
           PERFORM UNTIL WS-END-OF-FILE OR WS-BOOK-RETURNED = "Y"
               READ RETURN-FILE
                   AT END
                       MOVE "Y" TO WS-EOF-FLAG
                   NOT AT END
                       *> เปรียบเทียบ Borrow ID ว่าตรงกันหรือไม่
           ...
           *> Reset EOF flag หลังจากใช้งาน CHECK-IF-RETURNED
           MOVE "N" TO WS-EOF-FLAG.
